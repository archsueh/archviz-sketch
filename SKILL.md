...
name: archviz-sketch
description: |
  ⚠️ 需要出图 API，否则不可用。缺 image_generate 工具本 skill 无法出图，请先确认再加载。
...
> **视觉规范**：配图的色彩 / 线宽 / 留白遵循 **archviz-skills 共享设计系统**（`archviz-diagram` 的 `DESIGN.md`），与 `archviz-layout` 的四套视觉语言（静纸 / 实证 / 图桥 / 技术蓝图）保持调性一致。
